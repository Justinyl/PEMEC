...
# Parametric Sweep, Sensitivity Analysis
...
def set_inlet_req(m, Q0=100/120):
    m.Q0.unfix()
    m.Q0.fix(Q0)
    return

def set_pressure_req(m, Pc_0=100):
    m.Pc.unfix()
    m.Pc.fix(Pc_0)
    return


def check_stat(m, res):
    ...
    if (res2.solver.status == SolverStatus.ok):
        return 1  
    elif (res2.solver.termination_condition == TerminationCondition.infeasible):
        return 0
    else:
       return -1

def single_parametric_sweep(m, X):
    arr_stat = []
    j_list, P_list, ua_list = X
    l = len(j_list) + len(P_list) + len(ua_list)
    labels = ['Tm', 'j', 'H2', 'H2O', 'Hprod', 'cost', 'eta']
    dfs = []
    for i in range(0,l):
        dfs.append(pd.DataFrame(columns = labels))
    c = 0
    ...
    for javg in j_list:
        ...
        arr_stat.append(check_stat(m, res))
        ...
        for np in m.n:
            Tm_res.append(value(m.Tm[np]))
            j_res.append(value(m.j[np]))
            H2_res.append(value(m.Cc['H2',np]))
            H2O_res.append(value(m.Cc['H2O',np]))
            Hprod.append(value(m.Hprod_price))
            cost.append(value(m.cost))
            eta.append(value(m.eta_sys))
        dfs[c]['Tm'] = Tm_res
        dfs[c]['j'] = j_res
        dfs[c]['H2'] = H2_res
        dfs[c]['H2O'] = H2O_res
...
j_list = [1, 2, 3]
P_list = [80, 100, 120]
ua_list = [100/120*0.5,100/120*0.8, 100/120]
X = [j_list, P_list, ua_list]

print("============== Sweeping ==============")


dfs, arr_stat = single_parametric_sweep(m, X)
...
with pd.ExcelWriter('Model_V2_1_output.xlsx') as writer: 
    ...
    for df in dfs:
        msg = str(i)
        df.to_excel(writer, sheet_name=msg)
        i += 1
