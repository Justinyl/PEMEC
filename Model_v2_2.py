...
def flow_current_sweep(m, X, Y):
    """
    X: flowrate
    Y: current density
    """
    labels = ['Tm_avg', 'V', 'HProd', 'Eta_sys', 'Cost']
    dfs = [pd.DataFrame(columns = labels) for i in range(0,len(X))]
    c = 0
    for x in X:
        Tm_res = []
        V_res = []
        ...
        for y in Y:
            ...
            Tm_res.append(value(m.Tm[m.sl])-273.15)
            V_res.append(value(m.V))
            Hprod_res.append(value(m.Hprod_price))
            eta_res.append(value(m.eta_sys))
            cost_res.append(value(m.cost))

        dfs[c]['Tm_avg'] = Tm_res 
        dfs[c]['V'] = V_res
        dfs[c]['HProd'] = Hprod_res
        dfs[c]['Eta_sys'] = eta_res
        dfs[c]['Cost'] = cost_res
        c += 1
    with pd.ExcelWriter('Model_V2_2_output.xlsx') as writer: 
        for i,df in zip(range(0,len(X)), dfs):  
            msg = 'flowrate=' + str(Y[i])
...
X = [0.8*100/120,0.9*100/120,100/120,1.1*100/120,1.2*100/120]
Y = [0.7, 1, 2, 2.5, 3]

flow_current_sweep(m, X, Y)
...
print(value(m.eta_sys))
print(value(m.cost))
print(value(m.Hprod_price))
